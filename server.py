...
def fetch_bookings():
    with db_connection() as conn:
        cur = conn.execute("SELECT * FROM bookings ORDER BY id DESC")
        rows = cur.fetchall()
...
async def client_interface(error: str = ""):
    ...
    <div class="terms">
        ...
        <p>Upload the Letter of Instruction, ID, hospital records, RAF 1, RAF 4 and other supporting documents. Multiple files may be selected.</p>
        <input type="file" name="documents" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.png,.jpg,.jpeg,.tif,.tiff,.txt">
...
async def backend(request: Request):
    ...
    <html><head><title>Protean Backend</title>{CSS}</head><body><div class="container">
        ...
        <div class="notice">Microsoft Teams notification: {teams_status}. Uploaded files are stored in <b>{safe_text(UPLOAD_DIR)}</b>.</div>
