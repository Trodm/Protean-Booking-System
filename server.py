...
async def backend(request: Request):
    ...
    for row in rows:
        ...
        <td>
            <form action="/delete-booking?admin_key={ADMIN_PASSWORD}"
                  method="post"
                  onsubmit="return confirm('Delete this booking? This action cannot be undone.');">
                <input type="hidden" name="booking_id" value="{booking_id}">
                <button type="submit"
                        style="background:#b91c1c;color:white;padding:7px 10px;border:none;border-radius:5px;cursor:pointer;">
                    Delete
                </button>
            </form>
        </td>
    ...
    <div class="container">
        ...
        <div class="notice">Microsoft Teams Integration: {teams_status}</div>
